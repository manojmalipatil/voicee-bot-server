...
class TranslationManager:
    def __init__(self):
        self.translator_to_english = GoogleTranslator(source='ta', target='en')
        self.translator_to_tamil = GoogleTranslator(source='en', target='ta')
    ...
    def tamil_to_english(self, text: str) -> str:
        ...
        try:
            ...
            print(f"[TRANSLATION ERROR] Tamil to English: {e}")
            return text
    ...
    def english_to_tamil(self, text: str) -> str:
        ...
        try:
            if not text.strip():
                return text
            translated = self.translator_to_tamil.translate(text)
            return translated
        except Exception as e:
            print(f"[TRANSLATION ERROR] English to Tamil: {e}")
            return text
...
class DatabaseManager:
    ...
    def init_db(self):
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS grievances (
                ...
                original_language TEXT DEFAULT 'tamil',
                category TEXT,
                priority TEXT,
                sentiment TEXT,
                summary TEXT,
    ...
    def save_grievance(self, transcript: str, language: str = "tamil"):
...
class GrievanceTracker:
    ...
    def __init__(self, translator: TranslationManager):
        ...
        self.grievance_text_tamil = []  # Original Tamil
        self.grievance_text_english = []  # Translated English
        self.conversation_history = []
        self.word_count = 0
    
    def add_user_message(self, text: str):
        """Add user message in Tamil and translate to English."""
        # Store original Tamil
        self.grievance_text_tamil.append(f"பணியாளர்: {text}")
        
        # Translate to English and store
        english_text = self.translator.tamil_to_english(text)
        self.grievance_text_english.append(f"Employee: {english_text}")
        
        self.conversation_history.append({"role": "user", "content": text})
        self.word_count += len(text.split())
        
        print(f"[TRANSLATION] Tamil: {text[:50]}...")
    ...
    def get_full_grievance_tamil(self) -> str:
        """Get the complete grievance transcript in Tamil."""
        return "\n".join(self.grievance_text_tamil)
...
async def entrypoint(ctx: JobContext):
    ...
    session = AgentSession(
        ...
        stt=deepgram.STT(
            ...
            language="ta",  # Auto-detect language including Tamil
            smart_format=True,
            punctuate=True,
        ),
        llm=groq.LLM(
        ...
        tts=cartesia.TTS(
            model="sonic-3",
            voice="248be419-c632-4f23-adf1-5324ed7dbf1d",  # Use Tamil voice if available
            language="ta",  # Set TTS to Tamil
            speed=1.0,
        ),
    )
    ...
    try:
        ...
        full_grievance_tamil = grievance_tracker.get_full_grievance_tamil()
        ...
        print("-"*70)
        print("[ORIGINAL - TAMIL]")
        print(full_grievance_tamil)
        print("-"*70)
        ...
        db_manager.save_grievance(full_grievance_english, language="tamil")
        # ---------------------------------------------------
